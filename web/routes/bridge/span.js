...
router.get('/select',async function(req,res){
    ...
    //查构件表返回当前桥跨下的构件
    let sqlCom = `select * from mmp.tbl_bridge_component where BridgeLineID=${req.query.BridgeLineID} and SuperStructure='${resInfo[0].TypeName}' and StructureID=${req.query.BridgeSpanID}`
    try{
...
router.post('/insert',async function(req,res){
    let sql = `insert into mmp.tbl_bridge_span(BridgeLineID,SpanNum,SpanLength,SpanWidth,Clearance,ArticuleNum,SpanCombine) values(${req.body.BridgeLineID},'${req.body.SpanNum}',${req.body.SpanLength},${req.body.SpanWidth},'${req.body.Clearance}','${req.body.ArticuleNum}','${req.body.SpanCombine}')`;
    try{
...
router.get('/selType',async function(req,res){
    let sqlType = `select ComponentTypeName from mmp.componenttype where SuperStructure in (select TypeName from mmp.tbl_bridge_span)`;
    try{
